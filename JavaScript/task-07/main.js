...
const clickHandler = (event) => {
  ...
  const currentCount = productCount.get(productId) || 0;
  ...
  if (currentCount < 2) {
    ...
    price.textContent =
      (parseFloat(price.textContent) + Number(productPrice) * 0.5).toFixed(2) + ' $';
  }

  productCount.set(productId, currentCount + 1);
};
